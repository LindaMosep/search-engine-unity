...
{
    // Sansar sulo raporasyon
    // 32 bytes long.  Using a 16 character string here gives us 32 bytes when converted to a byte array.
    private const string initVector = "1234567891012345";
...
public class SaveSystem : MonoBehaviour
{
    ...
    protected static string encryptKey = "123456789101234567891012";
    ...
    public static void SavePlayer(Datalar data)
    {
    ...
    public static bool LoadSaveFile(ref Datalar data, string path)
    {
        var success = false;
        try
        {
            using (StreamReader reader = new StreamReader(path))
            {
                ...
                data = JsonUtility.FromJson<Datalar>(json);
                reader.Close();
                success = true;
            }
        }
    ...
    public static void LoadPlayer(ref Datalar data)
    {
        CreateFile();
    ...
    public void ImportPlayer2(DatalarAraci Datalar)
    {
        using (StreamWriter writer = new StreamWriter(Application.persistentDataPath + savePath))
        {
            ...
            {
                json = ConvertBase64ToString(reader);
                Datalar.data = JsonUtility.FromJson<Datalar>(json);
                reader.Close();
            }
        }
    }
